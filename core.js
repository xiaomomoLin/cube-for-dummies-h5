...
(function(root){
  ...
  function parseMove(token){const m=/^([URFDLB])(2|')?$/.exec(token);if(!m)throw Error('未知转动：'+token);return {f:m[1],turns:m[2]==='2'?2:m[2]==="'"?3:1};}
  ...
  function move(state,token){if(state.length!==54)throw Error('魔方需要 54 个色块。');const out=[];permutation(token).forEach((d,i)=>out[d]=state[i]);return out.join('');}
  ...
  function validate(values){
    const norm={};for(const f of FACES){norm[f]=normalize(values[f]||'');if(norm[f].length!==9||[...norm[f]].some(c=>!COLORS[c]))throw Error(`${NAMES[f]}面需要 9 个颜色，可用汉字或 W R G Y O B。`);}
    const centers=centersOf(norm);if(new Set(Object.values(centers)).size!==6)throw Error('六个面的中心颜色必须互不相同。');
    const all=Object.values(norm).join(''),counts=Object.fromEntries(Object.keys(COLORS).map(c=>[c,[...all].filter(x=>x===c).length]));
    if(Object.values(counts).some(n=>n!==9))throw Error('每种颜色应有 9 格。当前：'+Object.entries(counts).map(([c,n])=>COLORS[c].name+' '+n+'格').join('，'));
  ...
  function instruction(token,centers){
    const {f,turns}=parseMove(token),directions={R:['正面最右列向上翻','正面最右列向下翻'],L:['正面最左列向下翻','正面最左列向上翻'],U:['正面最上排向左移','正面最上排向右移'],D:['正面最下排向右移','正面最下排向左移'],F:['正面顺时针转，顶边向右','正面逆时针转，顶边向左'],B:['后层顶边向你的左手侧移动','后层顶边向你的右手侧移动']};
    return {title:`只拧${NAMES[f]}层 · ${turns===2?'180°':'90°'}`,detail:directions[f][turns===3?1:0]+(turns===2?'，连续转两次四分之一圈。':'，转四分之一圈。'),layer:`${COLORS[centers[f]].name}色中心所在的最外一层`,note:f==='B'?'后层离你最远，按你当前的左右判断；转后层时，正面九宫格不变。':'内层不动，整个魔方保持拿法不变。'};
  }
  ...
  function cornerLocation(state,target){const id=[...target].sort().join('');for(const [position,indices]of Object.entries(cornerIndices))if(indices.map(i=>state[i]).sort().join('')===id)return {position,indices};throw Error('找不到所选角块，请检查配色。');}
  function observe(before,after,target,centers){const old=cornerLocation(before,target),next=cornerLocation(after,target),solved=next.indices.every(i=>after[i]===FACES[Math.floor(i/9)]);return {from:CORNERS[old.position],to:CORNERS[next.position],target:CORNERS[target],status:solved?'位置和方向都正确':next.position===target?'位置正确，方向还未对齐':'尚未到目标位置',facing:next.indices.map(i=>COLORS[centers[after[i]]].name+'朝'+NAMES[FACES[Math.floor(i/9)]]).join(' · ')};}
  function rotateFace(raw){const a=normalize(raw);if(a.length!==9)throw Error('先填满这一面的 9 格，再旋转图案。');return [6,3,0,7,4,1,8,5,2].map(i=>a[i]).join('');}
